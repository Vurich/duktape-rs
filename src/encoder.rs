...
impl ::rustc_serialize::Encoder for Encoder {
    ...
    fn emit_nil(&mut self) -> EncodeResult {
        unsafe {
            duk_push_null(self.ctx.as_mut_ptr());
        }
        Ok(())
    }
    ...
    fn emit_f64(&mut self, v: f64) -> EncodeResult {
        unsafe { duk_push_number(self.ctx.as_mut_ptr(), v) };
        Ok(())
    }
    ...
    fn emit_enum_variant_arg<F>(&mut self, a_idx: usize, f: F) -> DuktapeResult<()>
    where
        F: FnOnce(&mut Encoder) -> DuktapeResult<()>,
    {
        unsafe {
        ...
        F: FnOnce(&mut Encoder) -> DuktapeResult<()>,
    {
        f(self).unwrap();
        unsafe {
            duk_put_prop_index(self.ctx.as_mut_ptr(), -2, idx as u32);
        }
        Ok(())
    }
        ...
        F: FnOnce(&mut Encoder) -> DuktapeResult<()>,
    {
        f(self).unwrap();
        unsafe {
            duk_safe_to_lstring(self.ctx.as_mut_ptr(), -1, null_mut());
        }
        Ok(())
    }
...
function assert_json(expected, value) {
    ...
    fn assert_json<T: DuktapeEncodable>(ctx: &mut Context, expected: &str, value: &T) {
        match ctx.call("assert_json", &[&expected, value]) {
            Ok(Value::Bool(true)) => {}
            Ok(Value::String(ref got)) => panic!("expected {:?}, got {:?}", expected, got),
    ...
    // Simple types.
    assert_encode!(1usize);
    assert_encode!(1u64);
    assert_encode!(1u32);
    assert_encode!(1u16);
    assert_encode!(1u8);
    assert_encode!(-1isize);
    assert_encode!(-1i64);
    assert_encode!(-1i32);
    assert_encode!(-1i16);
    assert_encode!(-1i8);
